...
def train_one_epoch(wandb_log, device_id, model: torch.nn.Module, criterion: DistillationLoss,
    ...
    prefix_token_num = 4
    vertex_indices = [1 + prefix_token_num, 
                14 + prefix_token_num,
                183 + prefix_token_num, 
                196 + prefix_token_num]
    ##########################################################################################################
    if args.cosub:
    ...
    for iteration, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if vertex_penalty: 
            outputs, all_tokens = model(samples, epoch, iteration, device_id)
        else: 
        ...
        if not args.cosub:
            ...
            if vertex_penalty: 
                # ----------------------------------------------------------
                #     꼭짓점 패치에 대한 추가 로스 corner_loss 예시
                # ----------------------------------------------------------
                # all_tokens shape: [B, num_patches+1+prefix_token_num, embed_dim]
                corner_patches = all_tokens[:, vertex_indices, :]  # [B, 4, embed_dim]

                # (예시) L2 노름이나 MSE 등 원하는 방식으로 추가 로스 계산
                # 보통 "특정 임베딩을 0에 가깝게" 만들거나, 
                # "특정 임베딩을 주어진 target vector에 가깝게" 만드는 용도로 많이 사용
                corner_loss = corner_patches.norm(dim=-1).mean()  
                
                # 알파는 하이퍼파라미터(얼마만큼 vertex patch를 강조하고 싶은지)
                alpha = 0.1
                loss = loss + alpha * corner_loss
                # ----------------------------------------------------------
