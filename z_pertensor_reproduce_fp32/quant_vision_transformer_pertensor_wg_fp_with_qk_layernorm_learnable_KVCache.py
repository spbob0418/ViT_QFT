...
class Attention(nn.Module):
    def __init__(
        ...
        ###############Learnable KV_Cache#################
        prefix_KCache_np = np.load("/home/shkim/QT_DeiT_small/reproduce/zz_prefix_patch_token_for_initialization/prefix_KCache.npy")
        prefix_KCache = torch.tensor(prefix_KCache_np, dtype=torch.float32)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        prefix_KCache = prefix_KCache[:,:,:4,:].to(device)
        self.learnable_kcache = nn.Parameter(prefix_KCache.clone()) #(1, 5, 384)

        prefix_VCache_np = np.load("/home/shkim/QT_DeiT_small/reproduce/zz_prefix_patch_token_for_initialization/prefix_VCache.npy")
        prefix_VCache = torch.tensor(prefix_VCache_np, dtype=torch.float32)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        prefix_VCache = prefix_VCache[:,:,:4,:].to(device)
        self.learnable_vcache = nn.Parameter(prefix_VCache.clone()) #(1, 5, 384)

        # self.learnable_kcache = nn.Parameter(torch.randn(1, 6, 5, 64))
        # self.learnable_vcache = nn.Parameter(torch.randn(1, 6, 5, 64))
        ##################################################
    ...
    def forward(self, x, epoch, iteration, device_id, prefix_token_num):
        ...
        ###########save for initialize#######################
        # if device_id == 0 :
            # prefix_KCache = k.cpu().detach().numpy()
            # prefix_VCache = v.cpu().detach().numpy()
            # print(k.size())#torch.Size([256, 6, 197, 64])
            # print(v.size())#torch.Size([256, 6, 197, 64])
        # # Define the filename for saving the .npy file
        #     filename1 = "/home/shkim/QT_DeiT_small/reproduce/zz_prefix_patch_token_for_initialization/prefix_KCache.npy"
        #     filename2 = "/home/shkim/QT_DeiT_small/reproduce/zz_prefix_patch_token_for_initialization/prefix_VCache.npy"
        #     # Save the NumPy array to an .npy file
        #     np.save(filename1, prefix_KCache)
        #     np.save(filename2, prefix_VCache)
        ...
        k_prefix_cache = self.learnable_kcache.expand(B, -1, -1, -1)
        v_prefix_cache = self.learnable_vcache.expand(B, -1, -1, -1)
        #cls token 과 image patch token 사이에 prefix_cache 삽입 
        #torch.cat 이용해서 prefix_cache  삽입하는 코드
        k_cls_token, k_image_patches = k[:, :, :1, :], k[:, :, 1:, :]
        k = torch.cat((k_cls_token, k_prefix_cache, k_image_patches), dim=2)  # (B, N + num_prefix_tokens, C)

        v_cls_token, v_image_patches = v[:, :, :1, :], v[:, :, 1:, :]
        v = torch.cat((v_cls_token, v_prefix_cache, v_image_patches), dim=2)  # (B, N + num_prefix_tokens, C)
        ######################################################
