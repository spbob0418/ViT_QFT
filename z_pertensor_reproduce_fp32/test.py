...
class _quantize_global(torch.autograd.Function):
    ...
    def forward(ctx, block_num, epoch, iteration, device_id, prefix_token_num, layer_info, x, w, bias=None, 
        ...
        print("input_quant:", input_quant.dtype)
        print("weight_quant:", weight_quant.dtype)
        output = torch.matmul(input_quant, weight_quant.t())
        ...
        # assert all(tensor.dtype == torch.float32 for tensor in [x_2d,w, input_quant, weight_quant, output, s_weight_quant, s_input_quant, s_o]), "Warning: One or more tensors are not of dtype torch.float32!"
    ...
    def backward(ctx, g_3D):
        ...
        print("a_g_2D_quant:", a_g_2D_quant.dtype)
        print("q_w:", q_w.dtype)

        grad_X = torch.matmul(a_g_2D_quant, q_w)
        print("grad_X:", grad_X.dtype)
        grad_X = grad_X * a_s_g_2D_quant * s_w 
        print("grad_X_after:", grad_X.dtype)
...
class lowbit_VisionTransformer(VisionTransformer):
    def __init__(self, abits, wbits, w_gbits, a_gbits,
        ...
        ####################White Patch Prefix#########################
        # prefix_token_np = np.load("/home/tsyeom/deit/zz_prefix_patch_token_for_initialization/raw_patch_token_768.npy")
