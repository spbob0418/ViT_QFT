...
class _quantize_global(torch.autograd.Function):
    ...
    def forward(ctx, block_num, epoch, iteration, device_id, prefix_token_num, layer_info, x, w, bias=None, 
        ...
        ################################
        if device_id ==0: 
            plot_tensor_distribution(x_2d, '8', layer_info, 'pertoken', iteration, 'Before Quantization')

        if prefix_token_num == None:
            input_quant, s_input_quant = a_qmodule(x_2d)
            ################################
            if device_id ==0: 
                plot_tensor_distribution(input_quant,'8', layer_info, 'pertoken', iteration, 'After Quantization')
            ...
            output = torch.matmul(input_quant, weight_quant.t()) ####inf 터짐 !!!!!!!!!!!!!!!3549455

            
            inf_count = (output == float('inf')).sum().item() + (output == -float('inf')).sum().item()
            print(f"Tensor에 있는 inf 값의 개수: {inf_count}, block num: {block_num}")
...
class Attention(nn.Module):
    ...
    def forward(self, x, epoch, iteration, device_id, prefix_token_num):
        ...
        q = q * self.scale #nan 18826368 값이 다양함 


        attn = (q @ k.transpose(-2, -1)) #nan 59610624
        ...
        attn = attn.softmax(dim=-1) #nan 59610624
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C) # nan 19365888
...
class lowbit_VisionTransformer(VisionTransformer):
    ...
    def forward(self, x, epoch=None, iteration=None, device_id=None):
        x, a = self.forward_features(x, epoch, iteration, device_id)
        # x = self.head(x)
...
def sixbits_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        abits = 6, wbits = 6, w_gbits = None, a_gbits = 6,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
