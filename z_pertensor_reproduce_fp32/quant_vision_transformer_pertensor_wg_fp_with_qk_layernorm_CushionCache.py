...
class _quantize_global(torch.autograd.Function):
    ...
    def backward(ctx, g_3D):
        ...
        # torch.autograd.set_detect_anomaly(True)  # 오류 추적 기능 활성화
        ...
        if ctx.fullprecision:
            ...
            elif w_g_qmodule.N_bits is None:
                grad_W = torch.matmul(g_2D.t(), fullprecision_x)
                grad_X = grad_X.view(reshape_3D[0],reshape_3D[1],-1)
            else: 
...
@register_model
def full_eightbits_deit_base_patch16_224(pretrained=False, finetune=None, register_num=0, **kwargs):
    model = lowbit_VisionTransformer(
