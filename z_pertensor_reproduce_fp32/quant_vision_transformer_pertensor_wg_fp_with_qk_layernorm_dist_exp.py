...
from tensor_distribution_plot import plot_tensor_distribution
import numpy as np
...
class Quantizer():
    ...
    def forward(self, x): 
        ...
        if self.symmetric:
            ...
            x = x / scale 
            x = round_pass(x)
            # plot_tensor_distribution(x, 'layer_id', 'quantization_id', 'iteration', 'After Q')
...
class _quantize_global(torch.autograd.Function):
    ...
    def forward(ctx, block_num, epoch, iteration, device_id, prefix_token_num, layer_info, x, w, bias=None, 
        ...
        ################################
        # if device_id ==0: 
        #     plot_tensor_distribution(x_2d, layer_info, 'pertensor', iteration, 'Before Quantization')

        if prefix_token_num == None:
            input_quant, s_input_quant = a_qmodule(x_2d)
            ################################
            # if device_id ==0: 
            #     plot_tensor_distribution(input_quant, layer_info, 'pertensor', iteration, 'After Quantization')
...
class Attention(nn.Module):
    ...
    def forward(self, x, epoch, iteration, device_id, prefix_token_num):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # nan 59606912
        
        # total_elements = attn.numel()  # 전체 요소 개수
        # num_inf = torch.isinf(attn).sum().item()  # inf 개수
        # num_nan = torch.isnan(attn).sum().item()  # nan 개수
        # inf_ratio = (num_inf / total_elements) * 100 if total_elements > 0 else 0
        # nan_ratio = (num_nan / total_elements) * 100 if total_elements > 0 else 0
        # print("total_elements", total_elements)
        # print("num_inf",num_inf)
        # print("inf_ratio", inf_ratio)
...
def fourbits_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        abits = 4, wbits = 6, w_gbits = None, a_gbits = 4,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def w4a6bits_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        abits = 6, wbits = 4, w_gbits = None, a_gbits = 4,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def sixbits_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        abits = 6, wbits = 4, w_gbits = None, a_gbits = 6,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
